...
void trim(std::string &string) {
    string.erase(string.begin(), std::find_if(string.begin(), string.end(), std::bind_front(std::not_equal_to<char>(), ' ')));
}

std::string remove_loop(const std::string &string) {
    std::string out = string.substr(string.find("[") + 1, string.length() - (string.find("[") + 1) - 1);
    trim(out);
    return out;
}
